...
public class TexturePacker : EditorWindow
{
    ...
    void SinglePackTexture()
    {
        ...
        {
            resPath = PathField(resPath, "图片资源路径", "设定为当前选择的路径", 100, 150);
            atlasPath = PathField(atlasPath, "保存图片路径", "设定为当前选择的路径", 100, 150);
            GUILayout.BeginHorizontal();
            {
        ...
        GUILayout.BeginVertical();
        {
            newCheckPath = PathField(newCheckPath, "新Atlas文件", "设定为当前选择的Atlas", 100, 150);
            oldCheckPath = PathField(oldCheckPath, "旧Atlas文件", "设定为当前选择的Atlas", 100, 150);
            GUILayout.BeginHorizontal();
            {
    ...
    private void BatchPackTexture()
    {
        GUILayout.BeginVertical();
        {
            batchResPath = PathField(batchResPath, "图片资源路径", "设定为当前选择的路径", 100, 150);
            batchAtlasPath = PathField(batchAtlasPath, "保存图片路径", "设定为当前选择的路径", 100, 150);
            GUILayout.BeginHorizontal();
            {
                GUILayout.Space((Screen.width - 150) / 2);
        ...
        GUILayout.BeginVertical();
        {
            batchNewCheckPath = PathField(batchNewCheckPath, "新Atlas路径", "设定为当前选择的路径", 100, 150);
            batchOldCheckPath = PathField(batchOldCheckPath, "旧Atlas路径", "设定为当前选择的路径", 100, 150);
            GUILayout.BeginHorizontal();
            {
    ...
    void ExportSpritePanel()
    {
        GUILayout.BeginVertical();
        {
            GUILayout.Label("先设定分割atlas图片保存路径，再选择要分割的atlas（可以多选），最后点分割atlas图片按钮");
            spriteFolderPath = PathField(spriteFolderPath, "分割atlas图片保存路径", "设定为当前选择的路径", 150, 150);
            GUILayout.BeginHorizontal();
            {
                GUILayout.Space((Screen.width - 150) / 2);
    ...
    void UpdateAtlasPanel()
    {
        GUILayout.BeginVertical();
        {
            GUILayout.Label("先把新加的图片拷贝到需要更新的Atlas的原图文件夹里或删除不需要的文件，再设定原图文件夹路径和Atlas文件路径，然后点击更新atlas按钮");
            updateResPath = PathField(updateResPath, "图片资源路径", "设定为当前选择的路径", 100, 150);
            updateAtlasPath = PathField(updateAtlasPath, "更新Atlas文件", "设定为当前选择Atlas文件", 100, 150);
            GUILayout.BeginHorizontal();
            {
                GUILayout.Space((Screen.width - 150) / 2);
